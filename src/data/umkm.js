export const umkmData = [
  {
    id: "opor-susu",
    name: "Opor Susu Legendaris",
    category: "Makanan & Minuman",
    rating: "4.4/5.0",
    description:
      "Pelopor opor ayam kuah susu di Jakarta! Menawarkan cita rasa opor unik yang gurih creamy, namun lebih ringan dari santan. Resep legendaris wajib coba.",
    location: "Cibulan Raya,Jakarta Selatan",
    imageUrl: "/pictUmkm/Screenshot from 2025-11-15 18-04-59.png",
    images: [
      "/pictUmkm/opor-1.jpg",
      "/pictUmkm/opor-2.jpg",
      "/pictUmkm/opor-3.jpg",
    ],
    coordinates: { lat: -6.24043, lng: 106.80852 },
    testimonials: [
      {
        quote:
          "Kirain bakal aneh opor pakai susu, ternyata gila! Gurihnya beda, lebih ringan tapi 'nendang'. Daging ayamnya empuk parah.",
        author: "Foodie Jaksel",
        highlight: "Gurihnya beda",
        highlightColor: "blue",
      },
      {
        quote:
          "Satu-satunya opor yang bikin kangen. Rasanya otentik tapi modern. Lontongnya juga pulen, pasangan yang pas banget.",
        author: "Keluarga Cibulan",
        highlight: "rasanya otentik",
        highlightColor: "green",
      },
    ],
  },
  {
    id: "warung-kopi-pak-budi",
    name: "Warung Kopi Pak Budi",
    category: "Makanan & Minuman",
    rating: "4.5/5.0",
    description:
      "Tempat 'ngopi' santai di Kemang dengan cita rasa kopi tubruk otentik. Menyajikan biji kopi lokal pilihan dan aneka camilan rumahan.",
    location: "Kemang, Jakarta Selatan",
    imageUrl: "/pictUmkm/warungKopiPaBudi.webp",
    coordinates: { lat: -6.2625, lng: 106.8164 },
    testimonials: [
      {
        quote:
          "Bukan kopi 'susu' kekinian. Ini beneran kopi! Kopi hitamnya nendang, tempatnya adem, pas buat kerja atau sekadar baca buku.",
        author: "Freelancer Kemang",
        highlight: "Ini beneran kopi!",
        highlightColor: "blue",
      },
      {
        quote:
          "Nongkrong di Kemang tapi harga bersahabat. Pisang gorengnya juga mantap, pasangan sempurna buat kopi tubruknya.",
        author: "Mahasiswa",
        highlight: "Harga bersahabat",
        highlightColor: "green",
      },
    ],
  },
  {
    id: "mbok-ndoro-mbloc",
    name: "Mbok Ndoro",
    category: "Makanan & Minuman",
    rating: "4.5/5.0",
    description:
      "Masakan Jawa otentik di M Bloc. Cicipi Nasi Langgi, Gudeg Jogja, dan Sego Pecel dalam suasana M Bloc yang hype dan kekinian.",
    location: "M BLOC SPACE Jl. Sisingsmangaraja 37 AH, Jakarta Selatan",
    imageUrl: "/pictUmkm/MbokNdoro.png",
    images: [],
    coordinates: { lat: -6.2263, lng: 106.8 },
    testimonials: [
      {
        quote:
          "Nongkrong di M Bloc tapi kangen masakan rumah? Ini jawabannya. Nasi Langgi-nya komplit dan rasanya medok. Juara!",
        author: "Pengunjung M Bloc",
        highlight: "Nasi Langgi-nya komplit",
        highlightColor: "green",
      },
      {
        quote:
          "Gudegnya 'legit' banget, manis gurihnya pas. Suasana M Bloc dapet, rasa Jogja-nya juga dapet. Perpaduan sempurna.",
        author: "Anak Jogja",
        highlight: "Gudegnya 'legit'",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "bakso-pak-mijan",
    name: "Bakso Pak Mijan",
    category: "Makanan & Minuman",
    rating: "4.6/5.0",
    description:
      "Bakso urat gerobakan legendaris di Bulungan. Dikenal dengan tekstur bakso yang 'kres', kuah kaldu sapi kaya rasa, dan tetelan melimpah.",
    location: "No Jl. Bulungan No.4 6, RT.6/RW.7, Kramat Pela, Jakarta Selatan",
    imageUrl: "/pictUmkm/BaksoPakMijan.png",
    images: [],
    coordinates: { lat: -6.2415, lng: 106.7995 },
    testimonials: [
      {
        quote:
          "Ini dia bakso urat paling juara se-Bulungan. Uratnya berasa, nggak cuma tepung. Kuahnya gurih, sambelnya mantap!",
        author: "Pecinta Bakso",
        highlight: "bakso urat paling juara",
        highlightColor: "green",
      },
      {
        quote:
          "Dari zaman sekolah SMA makannya di sini. Nggak berubah rasanya. Wajib minta tetelan, bikin kuahnya makin kaya rasa.",
        author: "Alumni 70",
        highlight: "Nggak berubah rasanya",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "warung-bu-vesti-barito",
    name: "Warung Bu Vesti Barito",
    category: "Makanan & Minuman",
    rating: "4.4/5.0",
    description:
      "Warteg 'naik kelas' legendaris di Barito. Surga bagi pencari lauk, puluhan menu masakan rumahan disajikan frisch setiap hari.",
    location: "Jl. Barito II No.4, RT.4/RW.4, Kramat Pela, Jakarta Selatan",
    imageUrl: "/pictUmkm/WarungBuVestiBarito.png",
    images: [],
    coordinates: { lat: -6.241, lng: 106.7998 },
    testimonials: [
      {
        quote:
          "Kalap! Itu kata pertama pas liat lauknya. Semua ada. Cumi, kikil, ayam, aneka sayur. Rasanya rumahan banget dan bersih.",
        author: "Karyawan Blok M",
        highlight: "Pilihan lauknya banyak",
        highlightColor: "blue",
      },
      {
        quote:
          "Makan siang andalan. Murah, kenyang, dan yang penting enak. Jam 12 udah pasti antri, tapi perputaran lauknya cepat jadi selalu frisch.",
        author: "Driver Ojol",
        highlight: "murah, enak, dan kenyang",
        highlightColor: "green",
      },
    ],
  },
  {
    id: "warung-taburai-kemang",
    name: "Warung Taburai - Kemang",
    category: "Makanan & Minuman",
    rating: "4.5/5.0",
    description:
      "Cita rasa otentik Manado di jantung Kemang. Spesialis Cakalang Fufu, Tinutuan, dan aneka masakan pedas yang menggugah selera.",
    location: "Jl. Kemang Utara No.8A, Bangka, Mampang Prpt., Jakarta Selatan",
    imageUrl: "/pictUmkm/WarungTaburai.png",
    images: [],
    coordinates: { lat: -6.261, lng: 106.8145 },
    testimonials: [
      {
        quote:
          "Pedasnya nampol! Cakalang Fufu-nya wangi asap, bumbunya berani. Ini baru masakan Manado asli, bukan abal-abal.",
        author: "Kawanua Jaksel",
        highlight: "Cakalang Fufu-nya terbaik",
        highlightColor: "green",
      },
      {
        quote:
          "Tempatnya 'cozy' dan bersih, beda dari resto Manado lain. Bawa keluarga pas weekend, semua suka. Perkedel jagungnya garing!",
        author: "Keluarga Kemang",
        highlight: "Tempatnya 'cozy'",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "sari-kebab",
    name: "Sari Kebab",
    category: "Makanan & Minuman",
    rating: "4.3/5.0",
    description:
      "Kebab 'sultan' di Kalibata. Bukan kebab biasa, isian dagingnya penuh, sayurnya segar, dan sausnya lumer. Jaminan puas!",
    location:
      "Jl. Wr. Jati Timur Raya No.33, RT.4/RW.3, Kalibata, Jakarta Selatan",
    imageUrl: "/pictUmkm/SariKebab.png",
    images: [],
    coordinates: { lat: -6.257, lng: 106.845 },
    testimonials: [
      {
        quote:
          "Ini bukan kebab, ini 'daging dibungkus'. Isinya beneran tumpah-tumpah, nggak pelit. Dagingnya tebel dan bumbunya meresap.",
        author: "Anak Kos Kalibata",
        highlight: "isian dagingnya banyak",
        highlightColor: "green",
      },
      {
        quote:
          "Selalu jadi penyelamat lapar malam. Ukuran jumbo-nya beneran jumbo, bisa buat berdua. Saus kejunya enak banget.",
        author: "Warga Lokal",
        highlight: "harganya juga terjangkau",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "bakso-88-moga-indah",
    name: "Bakso 88 Moga Indah",
    category: "Makanan & Minuman",
    rating: "4.2/5.0",
    description:
      "Warung bakso sederhana favorit warga Moga Indah. Menawarkan bakso halus super lembut dan kuah kaldu bening yang ringan menyegarkan.",
    location: "Moga Indah, Jakarta Selatan",
    imageUrl: "/pictUmkm/Bakso88MogaIndah.png",
    images: [],
    coordinates: { lat: -6.25384, lng: 106.822 },
    testimonials: [
      {
        quote:
          "Bakso halusnya... ya ampun, lembut banget! Nggak kerasa 'tepung'. Mie ayamnya juga enak, tipe yang bumbunya medok.",
        author: "Warga Kompleks",
        highlight: "Bakso halusnya lembut",
        highlightColor: "blue",
      },
      {
        quote:
          "Definisi 'comfort food'. Simpel, bersih, kuahnya ngaldu, dan harganya jujur. Selalu balik lagi tiap pengen bakso.",
        author: "Driver Online",
        highlight: "Simpel, bersih",
        highlightColor: "green",
      },
    ],
  },
  {
    id: "pisang-ijo-mertua-tebet",
    name: "Pisang Ijo Mertua, Tebet",
    category: "Makanan & Minuman",
    rating: "4.6/5.0",
    description:
      "Manis, lembut, dan segar! Es Pisang Ijo legendaris di Tebet ini menyajikan bubur sumsum premium dan saus sirup yang pas.",
    location:
      "Jl. Tebet Timur III N No.11, RT.8/RW.7, Tebet Tim., Jakarta Selatan",
    imageUrl: "/pictUmkm/PisangIjoMertua.png",
    images: [],
    coordinates: { lat: -6.2295, lng: 106.8528 },
    testimonials: [
      {
        quote:
          "Bubur sumsumnya itu lho... lembut, gurih, wangi pandan. Pisang ijonya juga pas, nggak keras. Selalu jadi favorit!",
        author: "Warga Tebet",
        highlight: "Bubur sumsumnya juara",
        highlightColor: "green",
      },
      {
        quote:
          "Pesen via ojol udah berkali-kali. Nggak pernah gagal. Manisnya pas, nggak bikin eneg. Es batunya juga dipisah, mantap.",
        author: "Pecinta Dessert",
        highlight: "Manisnya pas",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "kedai-mie-bangladesh-tebet",
    name: "Kedai Mie Bangladesh Tebet",
    category: "Makanan & Minuman",
    rating: "4.S/5.0",
    description:
      "Indomie 'naik kasta' dengan bumbu rempah otentik Bangladesh. Viral karena rasa 'nyemek' dan kaya rempah yang bikin nagih.",
    location:
      "Jl. Tebet Timur II No.31, RT.6/RW.5, Tebet Tim., Jakarta Selatan",
    imageUrl: "/pictUmkm/KedaiMieBangladeshTebet.png",
    images: [],
    coordinates: { lat: -6.229, lng: 106.852 },
    testimonials: [
      {
        quote:
          "Awas, antre! Tapi sepadan banget. Ini bukan Indomie biasa. Bumbunya 'medok' banget, rempahnya kuat. Gila, enak!",
        author: "Food Vlogger",
        highlight: "Bumbunya 'medok' banget",
        highlightColor: "green",
      },
      {
        quote:
          "Awalnya skeptis, ternyata beneran enak. Pesan yang 'tumis basah', bumbunya nempel semua di mie. Telornya setengah mateng, mantap.",
        author: "Mahasiswa",
        highlight: "worth it",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "wafa-gembel-99-tebet",
    name: "WAFA GEMBEL 99 TEBET TIMUR",
    category: "Makanan & Minuman",
    rating: "4.4/5.0",
    description:
      "Warung tenda pecel lele dan ayam 'gembel' yang sambalnya 'nampol' gila! Ayam garing dan sambal pedasnya jadi idola di Tebet.",
    location:
      "Jl. Tebet Timur Dalam Raya No.25-27 1, RT.1/RW.5, Tebet Tim., Jakarta Selatan",
    imageUrl: "/pictUmkm/WafaGembel99TebetTimur.png",
    images: [],
    coordinates: { lat: -6.2288, lng: 106.8535 },
    testimonials: [
      {
        quote:
          "SAMBELNYA! Itu aja. Pedesnya bikin keringetan tapi nagih. Ayam gembelnya garing, nasi uduknya wangi.",
        author: "Anak Kos Tebet",
        highlight: "Sambalnya mantap",
        highlightColor: "green",
      },
      {
        quote:
          "Lalapannya ambil sendiri sepuasnya, itu poin plus. Harga kaki lima, rasa sambelnya bintang lima. Penuh terus!",
        author: "Warga Lokal",
        highlight: "Lalapannya ambil sendiri",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "sate-pak-min-solo-tebet",
    name: 'Warung Sate Pak Min Solo "Sido Mampir"',
    category: "Makanan & Minuman",
    rating: "4.6/5.0",
    description:
      "Sate kambing empuk khas Solo di Tebet. Dagingnya nggak bau prengus, bumbu kecapnya meresap, dan tongsengnya juara.",
    location:
      "Jl. Tebet Timur Dalam II No.49, RT.3/RW.3, Tebet Tim., Jakarta Selatan",
    imageUrl: "/pictUmkm/WarungSatePakMinSolo.png",
    images: [],
    coordinates: { lat: -6.2275, lng: 106.8532 },
    testimonials: [
      {
        quote:
          "Tongsengnya! Dagingnya empuk, kuahnya 'legit' manis gurih. Sate kambingnya juga besar-besar, matang sempurna.",
        author: "Pecinta Sate",
        highlight: "daging kambingnya empuk",
        highlightColor: "green",
      },
      {
        quote:
          "Ini warung sate langganan bapak saya dari dulu. Sate buntelnya terbaik, bumbunya pas, nggak berlebihan.",
        author: "Keluarga Tebet",
        highlight: "Sate buntelnya favorit",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "bakmi-baba-ong",
    name: "Bakmi Baba Ong",
    category: "Makanan & Minuman",
    rating: "4.5/5.0",
    description:
      "Bakmi halal resep kuno dengan cita rasa otentik. Menggunakan mie kecil kenyal dan topping ayam kampung asli yang gurih.",
    location: "Jl. Tebet Raya No.87, Tebet Tim., Kec. Tebet, Jakarta Selatan",
    imageUrl: "/pictUmkm/BakmiBabaOng.png",
    images: [],
    coordinates: { lat: -6.2305, lng: 106.85 },
    testimonials: [
      {
        quote:
          "Akhirnya nemu bakmi halal yang 'rasa'-nya kayak bakmi non-halal. Mienya kecil-kecil, pas. Pangsitnya wajib coba!",
        author: "Bakmi Hunter",
        highlight: "Tekstur mie-nya pas",
        highlightColor: "blue",
      },
      {
        quote:
          "Tempatnya bersih, nuansa 'vintage'-nya dapet. Kuah kaldunya itu lho, gurih murni kaldu ayam. Beneran pake ayam kampung.",
        author: "Keluarga Muda",
        highlight: "Kuah kaldunya gurih",
        highlightColor: "green",
      },
    ],
  },
  {
    id: "uncle-thrift",
    name: "Uncle Thrift",
    category: "Fashion",
    rating: "4.7/5.0",
    description:
      "Surga tersembunyi para 'thrift hunter'. Menjual pakaian impor preloved berkualitas, dari jaket branded, crewneck, hingga kemeja unik.",
    location:
      "Jl. Siaga Swadaya No.3 8, RT.6/RW.4, Pejaten Bar., Ps. Minggu, Jakarta Selatan",
    imageUrl: "/pictUmkm/UncleThrift.png",
    images: [],
    coordinates: { lat: -6.2575, lng: 106.8436 },
    testimonials: [
      {
        quote:
          "Jangan bilang siapa-siapa, ini 'hidden gem' banget. Sering dapet harta karun di sini. Jaket brand US harga miring!",
        author: "Thrift Hunter",
        highlight: "branded dan harganya oke",
        highlightColor: "green",
      },
      {
        quote:
          "Tempatnya agak 'ngumpet', tapi koleksinya 'curated'. Nggak asal jual. Bersih dan udah di-laundry. Puas!",
        author: "Mahasiswa Fashion",
        highlight: "hidden gem",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "sj-laundry-damai-raya",
    name: "SJ Laundry & Dry Cleaning Damai Raya",
    category: "Jasa",
    rating: "4.6/5.0",
    description:
      "Solusi pakaian bersih, wangi, dan rapi di Pejaten. Melayani laundry kiloan hingga 'dry cleaning' jas dan kebaya dengan layanan antar-jemput.",
    location:
      "Jl. Damai No.3B 1, RT.1/RW.4, Pejaten Timur, Ps. Minggu, Jakarta Selatan",
    imageUrl: "/pictUmkm/SJLaundry.png",
    images: [],
    coordinates: { lat: -6.2599, lng: 106.8483 },
    testimonials: [
      {
        quote:
          "Setrikaannya juara! Rapi banget kayak baru beli. Wanginya juga tahan lama dan nggak apek. Langganan buat kemeja kantor.",
        author: "Ibu Rumah Tangga",
        highlight: "setrikaannya rapi",
        highlightColor: "green",
      },
      {
        quote:
          "Penyelamat pas ART mudik. Nggak cuma bersih, tapi lipatannya rapi. Jas saya juga aman di 'dry clean' sini.",
        author: "Karyawan Pejaten",
        highlight: "Bisa cuci cepat",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "savera-laundry",
    name: "SAVERA Laundry",
    category: "Jasa",
    rating: "4.5/5.0",
    description:
      "Laundry kiloan andalan anak kos Duren Tiga. Harga bersahabat, pengerjaan cepat, dan hasil bersih. Menerima cuci karpet dan boneka.",
    location:
      "Jl. Duren Tiga Selatan No.68, RT.8/RW.3, Duren Tiga, Kec. Pancoran, Jakarta Selatan",
    imageUrl: "/pictUmkm/SaveraLaundry.png",
    images: [],
    coordinates: { lat: -6.2483, lng: 106.8373 },
    testimonials: [
      {
        quote:
          "Murah, cepat, wangi. Tiga kata itu aja cukup. Nggak pusing lagi mikirin cucian numpuk. The best!",
        author: "Anak Kos Pancoran",
        highlight: "Harga kiloannya bersahabat",
        highlightColor: "blue",
      },
      {
        quote:
          "Nyoba cuci karpet di sini, hasilnya bersih banget! Debunya ilang semua. Harganya juga oke. Ramah pelayanannya.",
        author: "Warga Lokal",
        highlight: "Pelayanannya ramah",
        highlightColor: "green",
      },
    ],
  },
  {
    id: "eyelash-extension-dki-jakarta",
    name: "Eyelash Extension Dki Jakarta",
    category: "Kecantikan",
    rating: "4.8/5.0",
    description:
      "Bangun tidur langsung cantik! Studio 'eyelash extension' premium di Kemang. Pengerjaan detail, higienis, dan hasil natural.",
    location: "Jl. Kemang Utara G, Bangka, Kec. Mampang Prpt., Jakarta Selatan",
    imageUrl: "/pictUmkm/EyelashExtension.png",
    images: [],
    coordinates: { lat: -6.2588, lng: 106.8153 },
    testimonials: [
      {
        quote:
          "Hasilnya... GILA! Natural banget kayak bulu mata asli tapi lentik. Nggak perih, nggak ganjel. Terapisnya sabar.",
        author: "Beauty Enthusiast",
        highlight: "Hasilnya natural",
        highlightColor: "green",
      },
      {
        quote:
          "Tempatnya 'cozy' dan bersih banget. Udah coba 'lash lift' dan 'extension', dua-duanya tahan lama. Worth every penny.",
        author: "Model",
        highlight: "terapisnya profesional",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "it-support-jakarta",
    name: "IT SUPPORT JAKARTA SERVICE CENTER KOMPUTER PANGGILAN",
    category: "Jasa",
    rating: "4.9/5.0",
    description:
      "Laptop 'bluescreen'? Komputer lemot? Jasa servis komputer panggilan terpercaya. Teknisi andal, jujur, dan cepat datang ke rumah/kantor.",
    location:
      "6, Jl. Bangka IX No.24, RT.7/RW.12, Pela Mampang, Kec. Mampang Prpt., Jakarta",
    imageUrl: "/pictUmkm/ITSUPPORTJAKARTA.png",
    images: [],
    coordinates: { lat: -6.2505, lng: 106.818 },
    testimonials: [
      {
        quote:
          "Fast response banget! Dihubungi pagi, siang teknisi datang. Laptop kantor yang lemot parah jadi ngebut lagi. Jujur, nggak 'neko-neko'.",
        author: "Admin Kantor",
        highlight: "fast response",
        highlightColor: "blue",
      },
      {
        quote:
          "Penyelamat skripsi! Laptop tiba-tiba mati total. Data penting aman, dan biayanya transparan. Recommended!",
        author: "Mahasiswa",
        highlight: "Pengerjaannya jujur",
        highlightColor: "green",
      },
    ],
  },
  {
    id: "puspita-makmur-hidroponik",
    name: "Puspita Makmur Hidroponik",
    category: "Agribisnis",
    rating: "4.7/5.0",
    description:
      "Sayuran segar bebas pestisida langsung dari kebun hidroponik di Gandaria. Jual aneka selada, pakcoy, dan perlengkapan 'starter kit' hidroponik.",
    location:
      "Jl. Kri Beruang No.5 C 1, RT.4/RW.9, Gandaria Utara, Kec. Kby. Baru, Jakarta Selatan",
    imageUrl: "/pictUmkm/PuspitaMakmurHidroponik.png",
    images: [],
    coordinates: { lat: -6.2483, lng: 106.7915 },
    testimonials: [
      {
        quote:
          "Seladanya 'crunchy' dan segar banget! Beda sama yang di supermarket. Beli di sini buat bikin salad, rasanya jadi 'fresh'!",
        author: "Ibu Kompleks",
        highlight: "bebas pestisida",
        highlightColor: "green",
      },
      {
        quote:
          "Beli 'starter kit' hidroponik di sini. Diajarin dari nol sama owner-nya. Sekarang udah bisa panen sendiri. Seru!",
        author: "Pecinta Tanaman",
        highlight: "dikasih panduan lengkap",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "bimbingan-belajar-visi",
    name: "Bimbingan Belajar VISI",
    category: "Jasa/Pendidikan",
    rating: "4.7/5.0",
    description:
      "Bimbel legendaris di Barito dengan rekam jejak puluhan tahun. Fokus persiapan intensif masuk PTN, materi mendalam, dan 'try out' berkala.",
    location:
      "Jl. Barito II No.9, RT.8/RW.5, Kramat Pela, Kec. Kby. Baru, Jakarta Selatan",
    imageUrl: "/pictUmkm/BimbinganBelajarVISI.png",
    images: [],
    coordinates: { lat: -6.2439, lng: 106.8003 },
    testimonials: [
      {
        quote:
          "Anak saya lulus di UI berkat bimbingan di sini. Pengajarnya top, banyak alumni PTN. Materinya 'daging' semua, bukan cuma hafalan.",
        author: "Orang Tua Murid",
        highlight: "Pengajarnya berkualitas",
        highlightColor: "green",
      },
      {
        quote:
          "Metode belajarnya serius tapi asik. 'Try out'-nya beneran nguji kemampuan dan mirip soal aslinya. Nggak nyesel les di sini!",
        author: "Alumni 2024",
        highlight: "Try out-nya sangat membantu",
        highlightColor: "blue",
      },
    ],
  },
  {
    id: "micky-mocko-handicraft",
    name: "Micky Mocko Handicraft and Needlework",
    category: "Kerajinan",
    rating: "4.9/5.0",
    description:
      "Surga bagi para 'crafter' di Blok M! Toko terlengkap untuk segala kebutuhan sulam, rajut, jahit, dan aneka kerajinan tangan.",
    location:
      "Plaza Blok M, lantai 5, Jl. Bulungan no. 76, Kramat Pela, Kec. Kby. Baru, Jakarta Selatan",
    imageUrl: "/pictUmkm/MickyMockoHandicraft.png",
    images: [],
    coordinates: { lat: -6.2445, lng: 106.8006 },
    testimonials: [
      {
        quote:
          "Cari apa aja ada! Dari benang rajut impor, kain kanvas, manik-manik, sampai 'hoop' sulam. Surganya dunia per-craft-an.",
        author: "Crafter",
        highlight: "Surganya perajin",
        highlightColor: "green",
      },
      {
        quote:
          "Stafnya paham banget barang. Saya yang pemula nanya-nanya diladenin dengan sabar. Dikasih rekomendasi alat yang pas.",
        author: "Pemula Rajut",
        highlight: "Stafnya sangat paham",
        highlightColor: "blue",
      },
    ],
  },
];
